...
from .cross_att import CrossTransformer,FiLM
...
class DiffusionCondTrainingWrapper(pl.LightningModule):
    ...
    def __init__(
        ...
        if use_reconstruction_loss:
            ...
            if self.audio_out_channels == 2:
                self.loss_modules.append(
                    AuralossLoss(self.sdstft, 'audio_reals', 'audio_pred', name='mrstft_loss', weight=0.5), # Reconstruction loss
                )
            else:
                self.loss_modules.append(
        ...
        # self.crosstf=CrossTransformer(64, 256, 4, 4, 0.1, 32, 64)
        # self.crosstf=FiLM()
    ...
    def configure_optimizers(self):
        ...
        # Adjust learning rate for position cross_a2t parameters to be 10x higher
        # Create a new parameter group for position cross_a2t parameters with 10x learning rate
        position_params = []
        other_params = []
        # Get original learning rate from first param group
        orig_lr = opt_diff.param_groups[0]['lr']
        for name, param in self.diffusion.conditioner.conditioners.position.cross_a2t.named_parameters():
            position_params.append(param)
        
        # Remove position params from original groups
        for group in opt_diff.param_groups:
            group['params'] = [p for p in group['params'] if not any(torch.equal(p, pos_p) for pos_p in position_params)]
            
        # Add new group with higher learning rate
        opt_diff.add_param_group({
            'params': position_params,
            'lr': orig_lr * 10.0
        })
        print("Optimizer config:")
        print(f"Type: {type(opt_diff)}")
        print("\nParameter groups:")
        for i, group in enumerate(opt_diff.param_groups):
            print(f"\nGroup {i}:")
    ...
    def training_step(self, batch, batch_idx):
        ...
        # angle = torch.stack(angle) # n x 32 1 x 5
        # with torch.cuda.amp.autocast():
        #     noised_inputs_in = self.crosstf(angle.to(self.device), noised_inputs.to(self.device).permute(0,2,1)).permute(0,2,1)
        ...
        try:
            ...
            for cfg_scale in self.demo_cfg_scales:
                ...
                with torch.cuda.amp.autocast():
                    ...
                    # print("prompt", cond_inputs["cross_attn_cond"].shape)
