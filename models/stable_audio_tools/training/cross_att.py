...
class MultiheadCrossAttention(nn.Module):
    ...
    def forward(self, target, source, mask, tag):
        ...
        q = q.view(bsz, seq_len_q, self.num_heads, self.head_dim).permute(0, 2, 1, 3)  # Shape: [bsz, num_heads, seq_len, head_dim]
        k = k.view(bsz, seq_len_kv, self.num_heads, self.head_dim).permute(0, 2, 3, 1)  # Shape: [bsz, num_heads, head_dim, seq_len]
        v = v.view(bsz, seq_len_kv, self.num_heads, self.head_dim).permute(0, 2, 1, 3)  # Shape: [bsz, num_heads, seq_len, head_dim]
        ...
        attended_values = attended_values.permute(0, 2, 1, 3).reshape(bsz, seq_len_q, -1)  # Shape: [bsz, seq_len, embed_dim]
...
class CrossTransformer(nn.Module):
    def __init__(self, input_size, hidden_size, num_heads, num_layers, dropout_rate, msl_target=5, msl_source=6):
        super(CrossTransformer, self).__init__()
        self.embedding_t = nn.Linear(5, hidden_size)
        self.embedding_s = nn.Linear(64, hidden_size)
    ...
    def _generate_position_encoding(self, hidden_size, max_sequence_length):
        ...
        position = torch.arange(0, max_sequence_length, dtype=torch.float16).unsqueeze(1)
        div_term = torch.exp(torch.arange(0, hidden_size, 2).to(torch.float16) * (-math.log(10000.0) / hidden_size))
    ...
    def forward(self, target, source, mask=None, tag=None):
        target = self.embedding_t(target)
        source = self.embedding_s(source)
        target = target + self.position_encoding_target.to(target.device)[:target.shape[1]]
        source = source + self.position_encoding_source.to(source.device)[:target.shape[1]]
        for transformer_block in self.transformer_blocks:
            target, bad_mask = transformer_block(source, target, mask, tag)
        source = self.output_layer(source)
        return source
...
class FiLM(nn.Module):
    ...
    def __init__(self, input_dim=5, dim=64, output_dim=64, x_film=True):
    ...
    def forward(self, x, y):
        ...
        gamma, beta = torch.split(self.fc(film), self.dim, 2)
