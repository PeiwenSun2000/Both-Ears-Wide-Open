...
from .cross_transformer import CrossmodalTransformer
...
class Conditioner(nn.Module):
    def __init__(
        ...
        #             self.proj_out.weight.data.copy_(torch.eye(768, output_dim))
...
class PostionConditioner(Conditioner):
    def __init__(self, output_dim: int):
        super().__init__(output_dim, output_dim)
        self.total_time = 17
        self.cross_a2t = CrossmodalTransformer(output_dim, output_dim, 4, 4, 0.1, 128, self.total_time*4)
        # self.cross_a2t = CrossmodalTransformer(output_dim, output_dim, 4, 4, 0.1, self.total_time, 128)

    def create_gaussian_matrix(self, original, std_dev=1):
        m, n = original.shape
        
        # 找到每一列中具有最大值的行的索引
        max_indices = torch.argmax(original, dim=0)
        
        # 创建一个空的高斯矩阵
        gaussian_matrix = torch.zeros_like(original, dtype=torch.float)
        
        for col_index in range(n):
            center = max_indices[col_index].item()
            
            y = torch.arange(m).float()
            distribution = torch.exp(-((y - center) ** 2) / (2 * std_dev ** 2))
            distribution /= distribution.sum()
            
            gaussian_matrix[:, col_index] = distribution
    ...
    def audio_matrix(self, begin, end, speed=1):
        ...
        posi_begin, posi_end = int((begin-1)*4), int((end-1)*4)

        # Handle special cases
        if begin == end or speed == 0:
            water_matrix = torch.zeros((total_time, length))
            water_matrix[posi_begin][:] = 1
            return water_matrix

        if speed < 0:
            water_matrix = torch.zeros((total_time, length))
            water_matrix[posi_begin][:int(length*abs(speed))] = 1
            water_matrix[posi_end][int(length*abs(speed)):] = 1
            return water_matrix

        # Handle normal case
        linespace = torch.linspace(posi_begin, posi_end, int(length*speed), dtype=int)
        water_matrix = torch.zeros((total_time, length))
        frac_front = (length - int(length*speed)) // 2
        
        water_matrix[linespace, torch.arange(frac_front, len(linespace)+frac_front)] = 1
        water_matrix[posi_begin, :frac_front] = 1
        water_matrix[posi_end, len(linespace)+frac_front:] = 1
        return water_matrix

    def forward(self, cond_text, floats: tp.List[tp.List[tp.Tuple[float]]], device=None) -> tp.Any:
        ...
        for i in floats:
            matrices = []
            for params in i:
                matrix = self.audio_matrix(params[0], params[1], params[2])
                if params[3] != 0:
                    matrix = self.create_gaussian_matrix(matrix)
                matrices.append(matrix)
            matrices+=((4-(len(matrices)))*[torch.zeros_like(matrix)])
            matrices = torch.cat(matrices).to(device)
            all_matrices.append(matrices)
        all_matrices = torch.stack(all_matrices).to(device)
        all_matrices, _ = self.cross_a2t(cond_text,all_matrices)
        final_ret = [matrix for matrix in all_matrices]
        final_ret = torch.stack(final_ret)
        return [final_ret, torch.ones(final_ret.shape[0], 1).to(device)]
...
#         ret = []
#         for i in floats:
#             ret.append(self.audio_matrix(i[0],i[1],i[2]))
...
class CLIPConditioner(Conditioner):
    ...
    def forward(self, texts: tp.List[str], device: tp.Any = "cuda") -> tp.Any:
        ...
        return [self.proj_out(features), torch.ones(features.shape[0], 1).to(device)]
...
class T5Conditioner(Conditioner):
    ...
    def forward(self, texts: tp.List[str], device: tp.Union[torch.device, str]) -> tp.Tuple[torch.Tensor, torch.Tensor]:
        ...
        # a = torch.load("/data/peiwensun/project/encoder_hidden2.pt").cpu().to(device)
        # a_shape_0 = a.shape[0]
        # embeddings = torch.cat([a,torch.zeros(128-a_shape_0,768).to(device)],dim=0)
        # embeddings = embeddings.unsqueeze(0).repeat(8,1,1)
        # attention_mask[:,:a_shape_0]=True
        # attention_mask[:,a_shape_0:]=False
...
class VisualT5Conditioner(Conditioner):
    ...
    def __init__(
        ...
        raise NotImplementedError("VisualT5Conditioner is not ready for use yet")
        super().__init__(768, output_dim, project_out=project_out)
        self.enable_grad = enable_grad
        self.proj_out = self.proj_out.requires_grad_(enable_grad).to(torch.float16)


    def forward(self, paths: tp.List[str], device: tp.Any = "cuda") -> tp.Tuple[torch.Tensor, torch.Tensor]:
        raise NotImplementedError("VisualT5Conditioner is not ready for use yet")
        self.proj_out.to(device)
        embeddings,attention_mask=[],[]
        for i in paths:
            a = torch.load(i,map_location="cpu").to(device)
            a_shape_0 = a.shape[0]
            a = torch.cat([a,torch.zeros(128-a_shape_0,768).to(device)],dim=0).unsqueeze(0).to(device)
            embeddings.append(a)
            a_mask =torch.zeros(128, dtype=torch.bool).to(device)
            a_mask[:a_shape_0]=True
            attention_mask.append(a_mask.unsqueeze(0).to(device))
            ...
            for x in batch_metadata:
                ...
                if key == "position":
                    ...
                    if isinstance(x[condition_key], list) or isinstance(x[condition_key], tuple) and len(x[condition_key]) == 1:
                        ...
                        conditioner_inputs.append(x[condition_key])
            if key == "position":
                output[key] = conditioner(output["prompt"],conditioner_inputs, device)
            else:
...
def create_multi_conditioner_from_conditioning_config(config: tp.Dict[str, tp.Any]) -> MultiConditioner:
    ...
    for conditioner_info in config["configs"]:
        ...
        elif conditioner_type == "visualt5":
            conditioners[id] = VisualT5Conditioner(**conditioner_config)
        ...
        elif conditioner_type == "position":
            conditioners[id] = PostionConditioner(**conditioner_config)
