...
class ConditionedDiffusionModelWrapper(nn.Module):
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor, cond: tp.Dict[str, tp.Any], **kwargs):
        # print(x.shape,len(cond["prompt"]),cond["prompt"][0].shape)
...
class UNetCFG1DWrapper(ConditionedDiffusionModel):
    ...
    def forward(self,
        ...
        # print(x.shape,t.shape,cross_attn_cond.shape)
        outputs = self.model(
            x,
            t,
...
class DiTWrapper(ConditionedDiffusionModel):
    def __init__(
        ...
        **kwargs
        ...
        adapter_finetuning = False
        if adapter_finetuning:
            with torch.no_grad():
                for param in self.model.parameters():
                    param *= 0.5
        else:
            ...
            #     if param.requires_grad and "conditioner" in name:
