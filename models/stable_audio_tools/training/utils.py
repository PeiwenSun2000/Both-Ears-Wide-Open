...
def copy_state_dict(model, state_dict):
    ...
    print("------\n",set(model_state_dict.keys()) - set(state_dict.keys()),"\n------\n",len(set(state_dict.keys()) - set(model_state_dict.keys())),"\n------")
    print("------\n",set(state_dict.keys()) - set(model_state_dict.keys()),"\n------\n",len(set(state_dict.keys()) - set(model_state_dict.keys())),"\n------")
    # quit()
    # quit()
...
def copy_state_dict_naive(model, state_dict):
    """Load state_dict to model, but only for keys that match exactly.
    ...
    """
    model_state_dict = model.state_dict()
    fist_set = set(state_dict.keys()) - set(model_state_dict.keys())
    second_set = set(model_state_dict.keys()) - set(state_dict.keys())
    if True:
        new_dict = {}
        for key, value in state_dict.items():
            if key.startswith('diffusion.'):
        ...
        state_dict = new_dict
    for i in fist_set:
        if "model.encoder.layers.2.layers.0." in i:
            print(i)
